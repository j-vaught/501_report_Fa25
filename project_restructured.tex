...
This section compares the four Poisson solvers developed in Chapter~3. All solvers receive identical input---the same divergence field from photometric stereo---ensuring that performance differences are attributable to the solver, not the input data.

\subsubsection{Protocol: Same Divergence Field, Different Solvers}

To isolate solver behavior, we use a single photometric stereo run on the Gaussian test surface with eight lights. This produces a fixed gradient field $(p, q)$ and corresponding divergence $f = \partial_x p + \partial_y q$. Each solver integrates this same $f$ under its respective boundary conditions.

\begin{center}
\resizebox{0.95\linewidth}{!}{%
\begin{tikzpicture}[>=latex,
                    every node/.style={draw=USCGarnet, rounded corners, align=center,
                                       text width=3cm, fill=USCGarnet!30},
                    every path/.style={draw=USCGarnet, thick}]
\node (ps) {Gaussian + 8 lights\\$\to$ PS $\to (p,q)$};
...
\node[below right=1.5cm and -0.5cm of div] (neu) {FD Solver\\(Neumann BC)};
\node[right=2.5cm of div] (tik) {Tikhonov FFT\\($\lambda$ sweep)};
...
\subsubsection{Finite Difference Solver (Neumann BC)}

The Neumann solver enforces zero normal derivative at boundaries---the surface approaches edges with zero slope perpendicular to the boundary.
...
\begin{tabularx}{\textwidth}{lX}
\hline\hline
\textbf{Aspect} & \textbf{FD-Neumann Solver Characteristics} \\
\hline
\textbf{Boundary Condition} & Zero flux: $\partial z/\partial n|_{\partial\Omega} = 0$ \\
\textbf{Advantages} & Natural for unknown boundary heights; no artificial pinning \\
\textbf{Compatibility} & Requires $\iint f \, dA = 0$; mean is subtracted before solving \\
\textbf{Disadvantages} & Solution determined only up to additive constant; must mean-center \\
...
\subsubsection{Tikhonov Regularization Sweep}

Tikhonov regularization adds a smoothness penalty to suppress noise-induced artifacts at the cost of losing fine detail. We sweep the regularization parameter $\lambda \in [10^{-5}, 1]$ logarithmically.
...
\begin{tabularx}{\textwidth}{lX}
\hline\hline
\textbf{Aspect} & \textbf{Tikhonov Regularization Characteristics} \\
\hline
\textbf{Formulation} & Modified eigenvalues: $\hat{Z} = \hat{F} / (-k^2 - \lambda k^4)$ \\
\textbf{$\lambda$ Range} & $10^{-5}$ (minimal smoothing) to $1$ (heavy smoothing) \\
\textbf{Selection Method} & L-curve: plot $\|\nabla^2 z - f\|$ vs.\ $\|z\|$; choose corner \\
\textbf{Trade-off} & Small $\lambda$: preserves detail but amplifies noise; large $\lambda$: smooth but loses features \\
\hline\hline
\end{tabularx}
\end{table}

\noindent\textbf{Purpose:} The $\lambda$ sweep traces a U-curve where RMSE initially decreases (noise suppression) then increases (over-smoothing). The optimal $\lambda$ sits at the elbow, balancing fidelity and stability.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% End of Section 5: Experimental Validation
