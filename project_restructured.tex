...
Numerical solvers (FFT, finite difference) provide efficient solutions to the Poisson equation, but how do we know they are correct? Analytical solutions serve as ground truth for validating numerical implementations. By deriving closed-form expressions for simple test cases, we can compare numerical output against exact mathematics and quantify solver accuracy.
...
\end{equation}
The eigenfunctions $\sin(m\pi x/L_x)\sin(n\pi y/L_y)$ form an orthonormal basis for the solution space. Each eigenfunction satisfies the Laplacian eigenvalue problem with eigenvalue $-\lambda_{mn}$ where:
\begin{equation}
...
\subsubsection{Worked Example: Constant Source Term}

For a uniformly sloped plane, the gradient field has constant divergence $f(x,y) = f_0$. This test case admits a closed-form solution that validates our numerical pipeline.

Computing the Fourier coefficients of the constant function:
\begin{equation}
F_{mn} = \frac{4f_0}{L_x L_y} \int_0^{L_x} \sin\left(\frac{m\pi x}{L_x}\right) dx \int_0^{L_y} \sin\left(\frac{n\pi y}{L_y}\right) dy
\end{equation}
Each integral evaluates to $\frac{L}{\pi} \cdot \frac{1-\cos(m\pi)}{m}$, which is zero for even $m$ (since $\cos(m\pi) = 1$) and $\frac{2L}{m\pi}$ for odd $m$. Thus only odd indices contribute, and the modal amplitudes become:
\begin{equation}
A_{mn} = -\frac{16 f_0}{mn\pi^4} \cdot \frac{1}{\frac{m^2}{L_x^2}+\frac{n^2}{L_y^2}}, \quad m,n \text{ odd}; \qquad A_{mn}=0, \quad m \text{ or } n \text{ even}
\end{equation}
The $(1,1)$ mode dominates the reconstruction since higher modes decay rapidly. This analytical result provides ground truth for testing: we generate a synthetic test case with constant $f_0$, solve numerically with the FFT solver, and compare against the truncated series. Agreement to machine precision confirms correct implementation.
