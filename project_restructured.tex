...
Taking the first variation and setting it to zero yields the regularized normal equations:
\begin{equation}
(\nabla^4 + \lambda I) z = \nabla^2 f
\end{equation}
...
The regularization parameter $\lambda$ must balance bias against variance. Too small, and noise dominates; too large, and genuine surface features are smoothed away. Several methods exist for selecting $\lambda$:

\textit{L-curve method.} Plot solution norm $\|z_\lambda\|$ versus residual norm $\|\nabla^2 z_\lambda - f\|$ for various $\lambda$. The optimal $\lambda$ lies at the ``corner'' of this L-shaped curve, balancing the two objectives. This method requires no knowledge of the noise level.

\textit{Discrepancy principle.} If the noise standard deviation $\sigma$ is known, choose $\lambda$ such that the residual norm equals the expected noise level: $\|\nabla^2 z_\lambda - f\| \approx \sigma \sqrt{N}$. This ensures we fit the data to within noise tolerance without overfitting.

\textit{Cross-validation.} Hold out a subset of measurements, fit to the remainder, and evaluate prediction error on the held-out set. The $\lambda$ minimizing prediction error generalizes best.
